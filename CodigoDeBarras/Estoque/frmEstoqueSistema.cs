...
namespace CodigoDeBarras
{
    public partial class frmEstoqueSistema : Form
    {
        ...
        {
            InitializeComponent();
            CarregarGridEstoqueReal();
        }

        private void CarregarGridEstoqueReal()
        {
            EstoqueBLL estoqueBLL = new EstoqueBLL();
            List<Estoque> lsEstoque = new List<Estoque>();

            lsEstoque = estoqueBLL.ObterListaEstoque().ToList();

            lblQtdeReg.Text = lsEstoque.Count().ToString();
            lblUltAtualizacao.Text = lsEstoque.Count != 0 ? lsEstoque[0].DataModif.ToShortDateString() : "";

            dgvEstoqueReal.DataSource = lsEstoque;
        }
        ...
        private void frmEstoqueSistema_Load(object sender, EventArgs e)
        {
