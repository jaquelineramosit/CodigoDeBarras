            ...
            try
            {
                ...
                if (saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (lstEstoqueDivergente.Count > 0)
                    {
                        ...
                        PropertyInfo[] pi;
                        int k = 1;
                        pi = lstEnderecoFormatado[0].GetType().GetProperties();
                        foreach (var p in pi)
                        {
                        ...
                        for (var i = 0; i < lstEnderecoFormatado.Count; i++)
                        {
                            SistemaBrandMember.EnderecoFormatado enderecoFormatado = new SistemaBrandMember.EnderecoFormatado();
                            enderecoFormatado = lstEnderecoFormatado[i];
                            pi = enderecoFormatado.GetType().GetProperties();
                            int j = 1;
                            foreach (PropertyInfo p in pi)
                            {
                                ...
                                {
                                    string valor = "";
                                    if (p.GetValue(enderecoFormatado, null) != null)
                                    {
                                        valor = p.GetValue(enderecoFormatado, null).ToString();
                        ...
                        //Formata as celulas de vermelho ou amarelho, caso tenha alguma erro de quantidade de caracteres ou CEP
                        List<SelListaErroCampo_Result> lstErroCampoResult = new List<SelListaErroCampo_Result>();
                        DAL.LogErroCampo LogErroCampoDAL = new DAL.LogErroCampo();
                        lstErroCampoResult = LogErroCampoDAL.ObterLista();
                        Microsoft.Office.Interop.Excel.Range celulas;

                        foreach (var item in lstErroCampoResult)
                        {
                            celulas = excelApp.get_Range(item.Celula);
                            if (item.DescTipoErro == "CEP") celulas.Interior.Color = System.Drawing.Color.Yellow;
                            else celulas.Interior.Color = System.Drawing.Color.Red;
                        }
